...
sub new {
    ...
    $self->{'retransmits'}  = $h{'Retransmits'} ? $h{'Retransmits'} : 0;
    $self->{'localaddr'}    = $h{'LocalAddr'};
    $self->{'localport'}    = $h{'LocalPort'};
    ...
    my %io_sock_args = (
        ...
        LocalHost    => $self->{'localaddr'},
        LocalService => $self->{'localport'},
    );
    ...
    if ( $h{'NodeList'} ) {
        ...
        foreach my $node_a ( @{ $h{'NodeList'} } ) {
            ...
            if ( !scalar(@hostinfo) ) {
                print STDERR
                  "Can't resolve node hostname '$n_host': $! - skipping it!\n"
                  if $debug;
                next;
            }
            print STDERR "Adding "
              . inet_ntoa( $hostinfo[4] ) . ':'
              . $n_port
              . " to node list.\n"
              if $debug;
        ...
        if ($host) {
            ...
            if ( scalar(@hostinfo) ) {
                ...
                if ( exists( $self->{'node_list_a'}->{$act_addr_a} ) ) {
                    ...
                    print STDERR
                      "'$host' doesn't exist in node list - ignoring it!\n"
                      if $debug;
                }
            }
            else {
                print STDERR
"Can't resolve active node hostname '$host': $! - ignoring it!\n"
                  if $debug;
            }
        }
    }
    else {
        if ( $host !~ /^($RE{net}{IPv4}|$RE{net}{IPv6})$/ ) {
            my @hostinfo = gethostbyname($host);
            if ( !scalar(@hostinfo) ) {
                return $self->set_error( 'ESOCKETFAIL',
                    "Can't resolve hostname '" . $host . "'." );
            }
            $self->{'node_addr_a'} = inet_ntoa( $hostinfo[4] ) . ':' . $port;
        }
        else {
            $self->{'node_addr_a'} = $host;
            if ( $host =~ /^$RE{net}{IPv6}$/ ) {
                $self->{'node_addr_a'} = "[$host]";
            }
            $self->{'node_addr_a'} .= ':' . $port;
        }
    }
    if ($host) {
        $io_sock_args{'PeerAddr'} = $host;
        $io_sock_args{'PeerPort'} = $port;
        if ( exists $io_sock_args{'LocalAddr'} && !$io_sock_args{'LocalAddr'} )
        {
            delete $io_sock_args{'LocalAddr'};
        }
        if ( exists $io_sock_args{'LocalHost'} && !$io_sock_args{'LocalHost'} )
        {
            delete $io_sock_args{'LocalHost'};
        }
        if ( exists $io_sock_args{'LocalService'}
            && !$io_sock_args{'LocalService'} )
        {
            delete $io_sock_args{'LocalService'};
        }
        print STDERR "Got the SOCKET data: " . Dumper( \%io_sock_args )
          if $debug;
        $self->{'sock'} = IO::Socket::IP->new(%io_sock_args)
...
sub secret {
    my $self       = shift;
    my $new_secret = shift;
    $self->{'secret'} = $new_secret if $new_secret;
    return $self->{'secret'};
}
...
sub message_auth {
    my $self      = shift;
    my $new_state = shift;
    $self->{'message_auth'} = $new_state if defined $new_state;
    return $self->{'message_auth'};
}

sub authenticator {
    my $self      = shift;
    my $new_value = shift;
    $self->{'authenticator'} = $new_value if defined $new_value;
...
sub send_packet {
    ...
    elsif ($type == DISCONNECT_ACCEPT
        || $type == DISCONNECT_REJECT
        || $type == COA_ACK
        || $type == COA_NAK )
    {
        # don't touch authenticator yet
    }
    else {
        ...
        || ( $type == STATUS_SERVER ) )
    {
        ...
        $data =
            pack( 'C C n', $type, $request_id, $length )
          . $self->{authenticator}
          . $self->{attributes}
        ...
        $data =
          ...
          . $msg_authenticator;
        if ($debug) {
            print STDERR "RFC3579 Message-Authenticator: "
              . _ascii_to_hex($msg_authenticator)
              . " was added to request.\n";
        }
    }
    elsif (
        $self->{message_auth}
        && (   $type == DISCONNECT_ACCEPT
            || $type == DISCONNECT_REJECT
            || $type == COA_ACK
            || $type == COA_NAK )
      )
    ...
    if ( $self->{'no_connection'} ) { return $data; }
    ...
    if ( !defined( $self->{'node_list_a'} ) ) {
        if ($debug) {
            print STDERR 'Sending request to: ' . $self->{'node_addr_a'} . "\n";
        }
        $res =
        ...
        if ( !$retransmit && defined( $self->{'sock'} ) ) {
            if ($debug) {
                print STDERR 'Sending request to active node: '
                  . $self->{'node_addr_a'} . "\n";
            }
            $res = $self->{'sock'}->send($data)
              || $self->set_error( 'ESENDFAIL', $! );
        }
        else {
            if ($debug) {
                print STDERR "ReSending request to all cluster nodes.\n";
            }
            $self->{'sock'}      = undef;
            $self->{'sock_list'} = [];
            my %io_sock_args = (
            ...
            foreach my $node ( keys %{ $self->{'node_list_a'} } ) {
                if ($debug) {
                    print STDERR 'Sending request to: ' . $node . "\n";
                }
                $io_sock_args{'PeerAddr'} =
                  $self->{'node_list_a'}->{$node}->[0];
                $io_sock_args{'PeerPort'} =
                  $self->{'node_list_a'}->{$node}->[1];
                if ( exists $io_sock_args{'LocalAddr'}
                    && !$io_sock_args{'LocalAddr'} )
                {
                    delete $io_sock_args{'LocalAddr'};
                }
                if ( exists $io_sock_args{'LocalHost'}
                    && !$io_sock_args{'LocalHost'} )
                {
                    delete $io_sock_args{'LocalHost'};
                }
                if ( exists $io_sock_args{'LocalService'}
                ...
                my $new_sock = IO::Socket::IP->new(%io_sock_args)
                  or return $self->set_error( 'ESOCKETFAIL', $@ );
                $res =
                  $new_sock->send($data) || $self->set_error( 'ESENDFAIL', $! );
                if ($res) {
...
sub recv_packet {
    my ( $self, $detect_bad_id, $parse_data ) = @_;
    ...
    if ( !$self->{'no_connection'} ) {
        if ( defined( $self->{'sock_list'} )
            && scalar( @{ $self->{'sock_list'} } ) )
        {
            $sh = IO::Select->new( @{ $self->{'sock_list'} } )
              or return $self->set_error('ESELECTFAIL');
        }
        elsif ( defined( $self->{'sock'} ) ) {
            $sh = IO::Select->new( $self->{'sock'} )
              or return $self->set_error('ESELECTFAIL');
        }
        else {
        ...
        while ( $timeout > 0 ) {
            ...
            @ready      = $sh->can_read($timeout)
              or return $self->set_error( 'ETIMEOUT', $! );
            $end_time = time();
        ...
        if ( defined( $self->{'sock_list'} ) ) {
            ...
            if ($debug) {
                print STDERR "Registering new active peeer:"
                  . $self->{'node_addr_a'} . "\n";
            }
        }
    }
    ...
    ( $type, $id, $length, $auth, $resp_attributes ) =
      unpack( 'C C n a16 a*', $data );
    if ( $detect_bad_id && defined($id) && ( $id != $request_id ) ) {
        return $self->set_error('EBADID');
    }

    if ( !$self->{'authenticator'}
        && ( $type == COA_REQUEST || $type == DISCONNECT_REQUEST ) )
    {
        $self->gen_coa_authenticator( $type, $id, $length, $resp_attributes );
        $request_id = $id;
    }

    if ( $auth ne
        $self->calc_authenticator( $type, $id, $length, $resp_attributes ) )
    {
    ...
    if ( defined($rfc3579_msg_auth) ) {
        ...
        if ( $calc_hmac ne $rfc3579_msg_auth ) {
            if ($debug) {
                print STDERR
"Received response with INVALID RFC3579 Message-Authenticator.\n";
                print STDERR 'Received   '
                  . _ascii_to_hex($rfc3579_msg_auth) . "\n";
...
"Received response with VALID RFC3579 Message-Authenticator.\n";
    ...
    return ( $type, $auth ) if wantarray;
    return $type            if defined wantarray;
}
...
sub _decode_octets {
    ...
    #     $new_value .= unpack( 'C', $c );
    # }

    # if ($debug) {
    #     print STDERR $name." again: ".$new_value."\n";
    # }

    my $new_value = join( "", unpack( "H*", join( "", $value ) ) );
    $new_value =
      join( "", map { pack( 'C', hex($_) ) } ( $new_value =~ /(..)/g ) );
    if ($debug) {
        print STDERR $name . ": " . $new_value . "\n";
    }
    return $new_value;
}
...
sub _decode_value {
    ...
    if ( defined $type ) {
        if ( exists $decoder{$type} ) {
            ...
            if ($debug) {
                print {*STDERR}
                  "Unsupported type '$type' for attribute with id: '$id'.\n";
            }
        }
    }
    else {
        if ($debug) {
            print {*STDERR}
"Unknown type for attribute with id: '$id'. Check RADIUS dictionaries!\n";
        }
    }
...
sub get_attributes {
    my $self = shift;
    my (
        $vendor, $vendor_id, $name,     $id,  $length,
        $value,  $type,      $rawvalue, $tag, @a
    );
    ...
    while ( length($attrs) ) {
        ...
        if ( $id == ATTR_VENDOR ) {
            ( $vendor_id, $id, $length, $rawvalue ) =
              unpack( 'NCCa*', $rawvalue );
...
sub vendorID {
    ...
    if ( defined $attr->{'Vendor'} ) {
        return ( $dict_vendor_name{ $attr->{'Vendor'} }{'id'}
              // int( $attr->{'Vendor'} ) );
    }
    else {
        ...
        my $vendor_name = $dict_name{ $attr->{'Name'} }{'vendor'}
          or return NO_VENDOR;
        my $vendor_id = $dict_vendor_name{$vendor_name}{'id'}
          or return NO_VENDOR;
        return $vendor_id;
    }
}
...
sub _encode_enum {
    ...
    if (   defined( $dict_val{$name}{$value} )
        && defined( $dict_val{$name}{$value}{id} ) )
    {
...
sub _encode_tlv {
    ...
    foreach my $sattr ( sort { $a->{TLV_ID} <=> $b->{TLV_ID} } @{$value} ) {
        ...
        my $svalue =
          $self->_encode_value( $vendor, $sattr_id, $sattr_type, $sattr_name,
            $sattr->{Value} );
        if ( defined $svalue ) {
            $new_value .=
              pack( 'CC', $sattr_id, length($svalue) + 2 ) . $svalue;
        }
    }
...
sub _encode_value {
    ...
    if ( defined $type ) {
        if ( exists $encoder{$type} ) {
            ...
            if ($debug) {
                print {*STDERR}
"Unsupported type '$type' for attribute with name: '$name'.\n";
            }
        }
    }
    else {
        if ($debug) {
            print {*STDERR}
"Unknown type for attribute with name: '$name'. Check RADIUS dictionaries!\n";
        }
    }
...
# insert TLV type so we can order them by type inside of the container attribute
    ...
    for $a (@a) {
        $id = $dict_name{ $a->{Name} }{id} // int( $a->{Name} )
          unless ( exists( $a->{Nested} ) );
        $type = $a->{Type} // $dict_name{ $a->{Name} }{type}
          unless ( exists( $a->{Nested} ) );
        $vendor = vendorID($a);
        my $need_tag =
          ( defined $a->{Tag} ) || $dict_name{ $a->{Name} }{has_tag};
        if ($need_tag) {
            $a->{Tag} //= 0;
            if ( $a->{Tag} < 1 || $a->{Tag} > 31 ) {
                print STDERR "Tag value is out of range [1..31] for attribute "
                  . $a->{Name}
                  if $debug;
                next;
            }
            ...
            unless (
                defined(
                    $value = $self->_encode_value(
                        $vendor, $id, $type, $a->{Name}, $a->{Value}
                    )
                )
              )
            {
                print STDERR
"Unable to encode attribute $a->{Name} ($id, $type, $vendor) with value '$a->{Value}'\n"
                  if $debug;
                next;
            }
            print STDERR
"Adding attribute $a->{Name} ($id, $type, $vendor) with value '$a->{Value}'\n"
              if $debug;
            if ( $vendor eq NO_VENDOR ) {
                if ($need_tag) {
                    $self->{'attributes'} .=
                      pack( 'C C C', $id, length($value) + 3, $a->{Tag} // 0 )
                      . $value;
                }
                else {
                    $self->{'attributes'} .=
                      pack( 'C C', $id, length($value) + 2 ) . $value;
                }
            }
            else {
                # VSA
                ...
                if ( $vendor eq WIMAX_VENDOR ) {
                    ...
                    $value =
                      pack( 'N C C C', $vendor, $id, length($value) + 3, 0 )
                      . $value;
                }
                elsif ($need_tag) {
                    ...
                    $value = pack( 'N C C', $vendor, $id, length($value) + 2 )
                      . $value;
                }

                # add the normal RADIUS attribute header: type + length
                $self->{'attributes'} .=
                  pack( 'C C', ATTR_VENDOR, length($value) + 2 ) . $value;
            }
        }
        else {
            print STDERR "Adding nested VSAa\n" if $debug;
            if ( $vendor eq NO_VENDOR ) {
                print STDERR "Vendor not specified for nested VSAs\n" if $debug;
                next;
            }
            ...
            for my $n ( @{ $a->{Nested} } ) {
                $id   = $dict_name{ $n->{Name} }{id} // int( $n->{Name} );
                $type = $n->{Type} // $dict_name{ $n->{Name} }{type};
                $need_tag =
                  ( defined $n->{Tag} ) || $dict_name{ $n->{Name} }{has_tag};
                if ($need_tag) {
                    $n->{Tag} //= 0;
                    if ( $n->{Tag} < 1 || $n->{Tag} > 31 ) {
                        print STDERR
                          "Tag value is out of range [1..31] for attribute "
                          . $n->{Name}
                          if $debug;
                        next;
                    }
                ...
                unless (
                    defined(
                        $value = $self->_encode_value(
                            $vendor, $id, $type, $n->{Name}, $n->{Value}
                        )
                    )
                  )
                {
                    print STDERR
"Unable to encode attribute $n->{Name} ($id, $type, $vendor) with value '$n->{Value}'\n"
                      if $debug;
                    next;
                }
                ...
                if ($need_tag) {
                    ...
                    # tagged attribute
                    $value =
                      pack( 'C C C', $id, length($value) + 3, $n->{Tag} // 0 )
                      . $value;
                }
                else {
                    $value = pack( 'C C', $id, length($value) + 2 ) . $value;
                }
            ...
            $self->{'attributes'} .=
              pack( 'C C N', ATTR_VENDOR, length($full_value) + 6, $vendor )
              . $full_value;
        }
    }
...
sub replace_attr_value {
    ...
    while ( $cur_pos < $length ) {
        my ( $cur_id, $cur_len ) =
          unpack( 'C C', substr( $self->{'attributes'}, $cur_pos, 2 ) );
        if ( $cur_id == $id ) {
            if ( length($value) != ( $cur_len - 2 ) ) {
                if ($debug) {
                    print STDERR
"Trying to replace attribute ($id) with value which has different length\n";
                }
                last;
            }
...
sub gen_coa_authenticator {
    my ( $self, $type, $id, $length, $attributes ) = @_;

    $self->set_error;
    print "Setting CoA authenticator\n" if $debug;
    $self->{authenticator} = "\0" x 16;
}
...
sub gen_authenticator {
    ...
    $self->{'authenticator'} = pack "L4", rint(), rint(), rint(),
      rint();
}
...
# so we have to use a composite key
            ...
            if ( exists( $dict_name{$name} )
                and $dict_name{$name}{'type'} eq 'tlv' )
            {
...
sub dict_values {
    ...
    return defined $dict_val{$name} ? $dict_val{$name} : undef;
}
