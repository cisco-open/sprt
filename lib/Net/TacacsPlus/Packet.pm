...
sub new {
    ...
    if ( not exists $params{'version'} ) {
        ...
        $params{'version'} =
          $params{'major_version'} * 0x10 + $params{'minor_version'};
    }
    ...
    if ( $type == TAC_PLUS_AUTHEN ) {
        ...
        {
            $self->body(
                Net::TacacsPlus::Packet::AuthenStartBody->new(%params) );
        }
        elsif ( $params{'user_msg'} )    #else it is CONTINUE
        {
            $self->body(
                Net::TacacsPlus::Packet::AuthenContinueBody->new(%params) );
        }
        elsif ( $params{'status'} )      #else it is REPLY
        {
            $self->body(
                Net::TacacsPlus::Packet::AuthenReplyBody->new(%params) );
        }
        else { croak 'unknown request for body creation'; }
    }
        ...
        $self->body(
            Net::TacacsPlus::Packet::AccountRequestBody->new(%params) );
    }
    else {
...
sub check_reply {
    ...
    if ( ( $snd->seq_no() + 1 ) != ( $rcv->seq_no() ) ) {
        croak('seq_no mismash');
    }
    if ( ( $snd->session_id() ) != ( $rcv->session_id() ) ) {
        croak('session_id mismash');
    }
    if ( ( $snd->version() ) != ( $rcv->version() ) ) {
        croak('version mismash');
    }
...
sub decode_raw {
    ...
    my ( $raw_header, $raw_body ) = unpack 'a' . TAC_PLUS_HEADER_SIZE . 'a*',
      $raw_pkt;

    $self->header(
        Net::TacacsPlus::Packet::Header->new( 'raw_header' => $raw_header ) );
    ...
    if ( $self->seq_no % 2 == 0 ) {
        if ( $self->type == TAC_PLUS_AUTHEN ) {
            $self->body(
                Net::TacacsPlus::Packet::AuthenReplyBody->new(
                    'raw_body' => $raw_body
                )
            );
        }
            ...
            $self->body(
                Net::TacacsPlus::Packet::AuthorResponseBody->new(
                    'raw_body' => $raw_body
                )
            );
        }
            ...
            $self->body(
                Net::TacacsPlus::Packet::AccountReplyBody->new(
                    'raw_body' => $raw_body
                )
            );
        }
    ...
    # odd sequence numbers are received by the server
        ...
        if ( $self->type == TAC_PLUS_AUTHEN ) {
            $self->body(
                Net::TacacsPlus::Packet::AuthenStartBody->new(
                    'raw_body' => $raw_body
                )
            );
        }
            ...
            $self->body(
                Net::TacacsPlus::Packet::AuthorRequestBody->new(
                    'raw_body' => $raw_body
                )
            );
        }
            ...
            $self->body(
                Net::TacacsPlus::Packet::AccountRequestBody->new(
                    'raw_body' => $raw_body
                )
            );
        }
...
sub type_string {
    ...
    my %mapping = (
        ...
        0x02 => 'TAC_PLUS_AUTHOR',
        0x03 => 'TAC_PLUS_ACCT',
    );

    return $mapping{ $self->header->type(@_) } || $self->header->type(@_);
}
