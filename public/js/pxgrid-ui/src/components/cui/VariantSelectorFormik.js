...
const VariantSelectorFormik = ({
  ...
  const [curIdx, setIdx] = React.useState(() => {
    const idx = variants.findIndex(
      v =>
        v.selected ||
        v.variant === getIn(formik.values, `${varPrefix}.variant`, undefined)
    );
  ...
  React.useEffect(() => {
    const idx = variants.findIndex(
      v => v.variant === getIn(formik.values, `${varPrefix}.variant`, undefined)
    );
  ...
  React.useEffect(() => {
    if (curIdx >= 0) {
      formik.setFieldValue(`${varPrefix}.variant`, variants[curIdx].variant);
...
const refVariantSelectorFormik = connect(
  React.forwardRef((props, ref) => (
    <VariantSelectorFormik innerRef={ref} {...props} />
  ))
);
export { refVariantSelectorFormik as VariantSelectorFormik };
